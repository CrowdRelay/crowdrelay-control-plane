  ...
  return <PageShell>
    ...
    <div class="tenant-list"><For each={tenants.data?.items ?? []}>{tenant =>
      <Link to="/tenants/$slug" params={{ slug: tenant.slug }} class="tenant-row large">
        <div class="tenant-row-info"><strong>{tenant.displayName}</strong><small class="tenant-row-meta">{tenant.slug} · {tenant.workspaceId ? 'Workspace ready' : 'workspace pending'} · {tenant.regionalProfile ? `${tenant.regionalProfile.locale} · ${tenant.regionalProfile.timezone} · ${tenant.regionalProfile.dataRegion.toUpperCase()}` : 'regional profile unclassified'}</small></div>
