...
const kindLabel = (k: NotifierChannel['kind']) => k === 'discord' ? 'Discord webhook' : k === 'webhook' ? 'Generic webhook' : 'Email (relay)'
...
export function TenantNotifiersPage() {
  ...
  const create = useMutation(() => ({ mutationFn: () => api.createNotifier(slug(), { kind: kind(), label: label().trim(), url: kind() === 'email_relay' ? undefined : target().trim(), events: events(), enabled: true }), onSuccess: async () => { await refresh(); setNotice(`${label().trim()} added.`); setLabel(''); setTarget(''); setEvents([]) } }))
  ...
  const remove = useMutation(() => ({ mutationFn: (id: string) => api.deleteNotifier(slug(), id), onSuccess: refresh }))
  const test = useMutation(() => ({ mutationFn: async (id: string) => { try { const r = await api.testNotifier(slug(), id); return r.ok ? '' : (r.error ?? 'delivery failed') } catch (e) { return errorMessage(e, 'test delivery failed') } }, onMutate: (id) => setTestResult(c => ({ ...c, [id]: 'testing…' })), onSuccess: (err, id) => setTestResult(c => ({ ...c, [id]: err ? `failed: ${err}` : 'delivered ✓' })) }))
  ...
  return <section class="page">
    ...
    <Show when={channels.data} fallback={!channels.error ? null : undefined}>
      <article class="panel">
        ...
        <Show when={items().length === 0} fallback={<div class="notifier-list"><For each={items()}>{ch => <div class="notifier-row">
          <div class="notifier-meta"><strong>{ch.label}</strong><small>{kindLabel(ch.kind)} · {ch.config.to ?? ch.config.urlHost ?? 'endpoint'} · {ch.events.length ? ch.events.map(evLabel).join(', ') : 'all events'}</small><Show when={testResult()[ch.id]}><small class={testResult()[ch.id]?.includes('failed') ? 'notifier-test-bad' : 'notifier-test-ok'}>{testResult()[ch.id]}</small></Show></div>
    ...
    <form class="tenant-create-form" onSubmit={(e) => { e.preventDefault(); create.mutate() }}>
      ...
      <div class="form-grid">
        <label>Type<select value={kind()} onChange={(e) => { setKind(e.currentTarget.value as NotifierChannel['kind']); setTarget('') }}><option value="discord">Discord webhook</option><option value="webhook">Generic webhook</option><option value="email_relay">Email via platform relay</option></select></label>
