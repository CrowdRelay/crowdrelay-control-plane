...
export function AreaPage() {
  ...
  const overview = useQuery(() => ({ queryKey: ['area-overview', slug()], queryFn: () => api.areaOverview(slug()), refetchInterval: 15_000 }))
  const drops = useQuery(() => ({ queryKey: ['area-drops', slug()], queryFn: () => api.areaDrops(slug()), refetchInterval: 15_000 }))
  ...
  return <section class="page area-page">
    <div class="page-head">
      ...
      <div class="row-health"><Link class="ghost area-link-button" to="/tenants/$slug" params={{slug:slug()}}>Back to tenant</Link><Show when={overview.data}><StatusBadge status={overview.data!.enabled ? 'enabled' : 'disabled'} tone={overview.data!.enabled ? 'good' : 'muted'} /></Show></div>
    </div>
