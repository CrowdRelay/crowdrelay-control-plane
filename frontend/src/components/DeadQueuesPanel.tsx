...
export function DeadQueuesPanel(props: {
  ...
  return <>
    ...
    <For each={expandOutbox() ? (props.deadOutbox ?? []) : (props.deadOutbox ?? []).slice(0, DEAD_PREVIEW)}>{item => <Card class="mb-2.5 border-warning/30 bg-warning/10 p-4 last:mb-0">
    ...
    <Show when={props.isLoading}><SkeletonRows count={2} /></Show>
    <For each={expandDeliveries() ? (props.deadDeliveries ?? []) : (props.deadDeliveries ?? []).slice(0, DEAD_PREVIEW)}>{item => <Card class="mb-2.5 border-warning/30 bg-warning/10 p-4 last:mb-0">
      <div class="flex items-start justify-between gap-3 flex-wrap">
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-2 flex-wrap">
            <Badge variant="warning">{item.event_type.replace(/_/g, ' ')}</Badge>
    ...
    <Show when={props.isLoading}><SkeletonRows count={2} /></Show>
    <For each={expandPush() ? (props.deadPush ?? []) : (props.deadPush ?? []).slice(0, DEAD_PREVIEW)}>{item => <Card class="mb-2.5 border-warning/30 bg-warning/10 p-4 last:mb-0">
      <div class="flex items-start justify-between gap-3 flex-wrap">
        <div class="min-w-0 flex-1">
          <div class="flex items-center gap-2 flex-wrap">
