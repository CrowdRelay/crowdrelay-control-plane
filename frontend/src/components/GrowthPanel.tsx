...
export function GrowthPanel(props: { growth: GrowthOverview | null | undefined; degraded: boolean }) {
  ...
  const actionGuidance = () => {
    switch (needsAction()) {
      case 'stalled': return 'Campaigns are queued but no delivery worker is claiming them. Check that the n8n growth delivery workflows are active and pointed at this tenant.'
      case 'disabled': return 'Campaign delivery is switched off. Toggle delivery in the Runtime switches section above to let workers pick up the queue.'
      case 'failed': return 'Some deliveries failed permanently. Open the Operator Attention page to inspect and retry them.'
      default: return null
    }
  }
  ...
  return <Card class="p-5 operations-panel">
    <div class="flex items-start justify-between gap-4 mt-6 mb-3">
      <div>
        ...
        <p class="mt-1 text-sm text-muted-foreground">CrowdRelay queues growth campaigns; external n8n workers deliver them.</p>
      </div>
      ...
      <Show when={growth.data} fallback={!growth.error ? <div class="mini-skeleton"/> : null}>{data => <>
        <Show when={data().totals.stalled_campaigns > 0}>
          <div class="rounded-lg border border-destructive/30 bg-destructive/10 p-3 flex flex-col gap-1">
            ...
            <span class="text-sm text-secondary-foreground">{data().totals.stalled_campaigns} campaign(s) waiting for a delivery worker.</span>
          </div>
        </Show>
        <Show when={!data().campaigns_enabled && data().totals.scheduled_campaigns > 0}>
          <div class="rounded-lg border border-destructive/30 bg-destructive/10 p-3 flex flex-col gap-1">
            ...
            <span class="text-sm text-secondary-foreground">{data().totals.scheduled_campaigns} scheduled campaign(s) cannot be delivered while delivery is off.</span>
          </div>
        </Show>

        {/* Actionable guidance — what to do when the badge is not "good". */}
        <Show when={needsAction()}>
          <div class="mt-3 p-3.5 border border-warning/30 bg-warning/10 rounded-lg">
            <strong class="text-foreground">What to do</strong>
            <p class="mt-1 m-0 text-sm text-secondary-foreground">{actionGuidance()}</p>
          </div>
        </Show>
        ...
        <section class="mt-6 pt-6 border-t border-border">
          ...
          <div class="grid grid-cols-2 gap-3 mt-3">
            <div class="p-3 border border-border rounded-lg bg-card"><strong class="block text-lg font-bold tabular-nums text-foreground">{count(outreach()?.eligible_playlist_targets)}</strong><span class="block text-xs text-muted-foreground">eligible playlist targets</span></div>
            <div class="p-3 border border-border rounded-lg bg-card"><strong class="block text-lg font-bold tabular-nums text-foreground">{count(outreach()?.suppressed_targets)}</strong><span class="block text-xs text-muted-foreground">suppressed targets</span></div>
          </div>
          <Show when={outreach() && outreach()!.eligible_playlist_targets === 0 && outreach()!.playlist_opportunities === 0}>
            <div class="mt-3"><EmptyState label="No playlist targets" hint="Targets appear once the detector scans for them." /></div>
          </Show>
        </section>
        ...
        <section class="mt-6 pt-6 border-t border-border">
          ...
          <Show when={data().campaigns.length > 0} fallback={<div class="mt-3"><EmptyState label="No growth campaigns" hint="Campaigns appear once the intelligence creates them." /></div>}>
