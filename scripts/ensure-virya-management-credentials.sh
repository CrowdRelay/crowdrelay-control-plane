...
[[ "$management_url" == "http://crowdrelay-api-active:8080" ]] || fail 'Control Plane management URL is not canonical'
...
cd "$root"
fail() { printf 'ERROR: %s\n' "$*" >&2; exit 1; }
# Resolve the live API container by the alias that survives a colour swap.
#
# This script asserted the canonical management URL two checks above — and then
# inspected `crowdrelay-api-blue-1` and `crowdrelay-api-green-1` by name, which
# is the one thing the deploy discipline forbids: those names change on every
# swap, and production currently runs neither (it is `crowdrelay-api-1`). The
# preflight therefore reported "no CrowdRelay API container running" against a
# healthy API and blocked the Control Plane phase of every ecosystem deploy.
#
# `crowdrelay-api-active` is the invariant. Ask Docker which container carries
# it rather than guessing at the colour.
api_container() {
  local name
...
api_name="$(api_container || true)"
[[ -n "$api_name" ]] || fail 'no CrowdRelay API container carries the crowdrelay-api-active alias'
runtime_env="$(docker inspect "$api_name" --format '{{range .Config.Env}}{{println .}}{{end}}' 2>/dev/null || true)"
[[ -n "$runtime_env" ]] || fail "could not read the environment of $api_name"
...
text=upsert(text,'CONTROL_PLANE_VIRYA_MANAGEMENT_URL','http://crowdrelay-api-active:8080')
...
api_name="$(api_container || true)"
[[ -n "$api_name" ]] || fail 'no CrowdRelay API container carries the crowdrelay-api-active alias'
running_image="$(docker inspect "$api_name" --format '{{.Config.Image}}' 2>/dev/null || true)"
[[ -n "$running_image" ]] || fail "could not read the image of $api_name"
...
new_name="$(api_container || true)"
new_image="$(docker inspect "$new_name" --format '{{.Config.Image}}' 2>/dev/null || true)"
[[ "$new_image" == "$running_image" ]] || fail "credential reload changed CrowdRelay release: before=$running_image after=$new_image"
image_id="$(docker inspect "$new_name" --format '{{.Image}}' 2>/dev/null || true)"
