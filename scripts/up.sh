...
CONTROL_PLANE_VIRYA_SIGNAL_URL=https://virya.music/signal
...
# Sync telemetry credentials so crowdrelayctl heartbeat can report runtime
# status back to this control plane. Without these, heartbeat silently
# returns early and the dashboard shows the tenant as stale forever.
# These go into .crowdrelay.local.sh (sourced by crowdrelayctl) rather than
# .env (sourced by docker-compose) because crowdrelayctl reads its config
# from the local file, not the compose env file.
CR_LOCAL="${CROWDRELAY_DIR}/.crowdrelay.local.sh"
cat > "$CR_LOCAL" << 'TELEMETRY_EOF'
# Auto-generated by crowdrelay-control-plane/scripts/up.sh.
# Telemetry credentials for crowdrelayctl heartbeat → local control plane.
CROWDRELAY_CONTROL_PLANE_BASE_URL=http://127.0.0.1:8090
CROWDRELAY_CONTROL_PLANE_TENANT_SLUG=virya
# Use the local dev compose file so heartbeat's compose exec targets the
# running containers (docker-compose.yml, not compose.production.yaml).
CROWDRELAY_COMPOSE_FILE=docker-compose.yml
CROWDRELAY_ENV_FILE=.env
TELEMETRY_EOF
# Append the token separately (it's a secret, don't put it in the heredoc
# where it would be visible in the script's own source).
sed_inplace "s|^# Auto-generated.*|# Auto-generated by crowdrelay-control-plane/scripts/up.sh. Do not commit.|" "$CR_LOCAL"
echo "CROWDRELAY_CONTROL_PLANE_TELEMETRY_TOKEN=$CONTROL_PLANE_TELEMETRY_TOKEN" >> "$CR_LOCAL"
chmod 600 "$CR_LOCAL"
...
endpoints=(
  ...
  "automation events|/tenants/virya/automation/events"
  "automation workflows|/tenants/virya/automation/workflows"
