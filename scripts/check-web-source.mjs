...
if (!authSource.includes('Basic ${btoa(binary)}') || /localStorage|sessionStorage/.test(authSource)) throw new Error('operator Basic credential must be memory-only')
...
console.log('CONTROL_PLANE_WEB_SOURCE=PASS auth=styled-edge-basic+server-bearer runtime-health=server-authoritative provisioning=create+deploy+lifecycle')
