...
cleanup() {
  ssh -T "$HOME_HOST" "rm -f '$REMOTE_AREA'" >/dev/null 2>&1 || true
}
trap cleanup EXIT
...
[[ -f "$AREA_SOURCE" && ! -L "$AREA_SOURCE" ]] || fail "missing or unsafe canonical management file: $AREA_SOURCE"
...
report="$(ssh -T "$HOME_HOST" sudo bash -s -- "$HOME_DIR" "$REMOTE_AREA" "$area_sha" <<'REMOTE'
set -Eeuo pipefail
root="$1"
area_source="$2"
expected_area_sha="$3"
...
printf 'CANONICAL_MANAGEMENT_OVERLAY=PASS area_sha256=%s\n' "$runtime_area_sha"
REMOTE
...
for attempt in $(seq 1 15); do
  ...
  printf '... management path not ready yet attempt=%s/15\n' "$attempt" >&2
done
